...
from botConfig import microcenter_xPaths, additional_settings
...
def check_in_stock(item_link,driver):
    try:
        ...
        add_to_cart_button = driver.find_element(microcenter_xPaths["addToCart"]).click()
        print("Item Found")
        return True
...
def purchase_item(item_link,driver):
    view_cart = driver.find_element(microcenter_xPaths["viewInCart"]).click()
    random_wait_time()
    checkout = driver.find_element(microcenter_xPaths["checkOut"]).click()
