...
from botConfig import bestbuy_xPaths, additional_settings
...
def check_in_stock(item_link,driver):
    try:
        ...
        add_to_cart_button = driver.find_element(bestbuy_xPaths["addToCart"]).click()
        print("Item Found")
        return True
...
def purchase_item(item_link,driver):
    checkout = driver.find_element(bestbuy_xPaths["checkout"]).click()
    random_wait_time()
    goto_payment = driver.find_element(bestbuy_xPaths["toPayment"]).click()
    random_wait_time()
    place_order = driver.find_element(bestbuy_xPaths["placeOrder"]).click()
